...
void init_task()
{
    WVAR(readPt,  (uint8_t *) READ_ADDR);       // Write
    WVAR(writePt, (uint8_t *) WRITE_ADDR);     // Read
    WVAR(buf,0);
    WVAR(bufIdx ,16);
    WVAR(byte_flag, 1);
...
void task_write()
{
    ...
    uint16_t inbufIdx = RVAR(bufIdx);
    uint8_t *inwritePt = RVAR(writePt);
    uint16_t inbuf = RVAR(buf);
