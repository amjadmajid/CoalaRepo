...
Checkpointing volatile state alone do not ensure data consistency when the system can directly manipulate non-volatile memory~\cite{mspcdino}.
...
%\end{figure}
To understand the expense of non-volatile memory access, we measured the energy consumption of volatile (SRAM) and non-volatile (FRAM) memory 
...
\textbf{Incorrectly Sized Tasks.} Compiler-inserted checkpoints or programmer-defined tasks can both be incorrect or inefficient.
...
Consequently, existing systems either risks non-termination to achieve high performance, or 
use conservatively small tasks that result in poor performance. 
When {\em porting} apps from one device to another, the problem gets much worse.
An excessively costly task on a device with a small energy buffer may be a relatively short task on a device with a larger buffer, and vice versa. 
Buffer sizes from prior work on intermittently-powered computing hardware vary widely from 20\,$\mu $F~\cite{rodriguez_tbcs_2015} to 0.1\,F~\cite{moo},
exemplifying the problem.
%
As a solution, \sys's execution model is based on {\em task coalescing} and {\em task downscaling}.  
Coalescing dynamically merges multiple tasks based on execution behavior to amortize the overhead between tasks if tasks are small.
Downscaling breaks up a single task into multiple subtasks that commit individually to avoid executing a single task that is too 
large to complete using the device's buffered energy.  Section~\ref{sec:task_adaptation} describes \sys's adaptive coalescing and 
downscaling mechanisms in detail. The main enabler for \sys's adaptation is its novel \emph{memory virtualization} mechanism, 
which is described in Section~\ref{sec:memory_virtulaization}.
