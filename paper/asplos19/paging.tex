...
\begin{figure}
	...
	\caption{induction of WAR dependency by coalescing: without extra protection, which static runtime cannot reason about, task coalescing induces WAR dependency (at variable $x$) even though individual tasks are WAR protected.}
	\label{figure:coalescedWar}
\end{figure}

\sys's {\em virtual memory manager} (VMM) is a core enabler of \sys task adaptation mechanism. Its necessity is illustrated by example given in Figure~\ref{figure:coalescedWar}: even though each task might be WAR-protected, coalescing breaks the global WAR insurance. Therefore we need a new memory management mechanism that will keep WAR-protection despite coalescing, which in case of \sys is exemplified by VMM. 
