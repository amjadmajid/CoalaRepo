...
\xxx{add \cite{intel} work; separate by-programmer and by-compiler; both cases
hit the energy estimation problem; also intel work proposes a method for decomposing
into one of many decompositions, but does not give a good solution for choosing one}
...
Unfortunately, naively merging two atomic tasks might produce a non-atomic task, that may leave non-volatile memory inconsistent after a partial execution. A merge breaks atomicity when atomic merged tasks form a \emph{write-after-read (WAR)} dependency---for instance if two tasks \texttt{\{x++\}; \{vector[x]=v;\}} are merged and if the power failure occurs after \texttt{x++}, the value \texttt{x} will be increased twice when the merged task is restarted, that leads to an inconsistency.
%
...
\begin{itemize}
	\item \xxx{Two} adaptive coalescing strategies for selecting boundaries to coalesce based on past execution behaviour;
