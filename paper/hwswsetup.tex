...
board.
%
Every benchmark listed in the next section was run repeatedly for X\,s on
each platform and the result was averaged for the duration of the experiment.
\todo{check if this is correct}{Carlo}
...
%
We powered the WISP using an RF signal generator~\cite{} \todo{Add reference
to signal generator}{Przemek} emitting 20\,dBm sinusoidal wave at 915\,MHz.
...
experiments.
%
To obtain execution time, the software toggled GPIO pins at the beginning and
end of the application, and the Saleae~\cite{saleae} logic analyzer measured
...
TI~\cite{ti-fft}.
%
\todo{add reference}{Przemek}
%
All applications were compiled using TI GCC~\cite{ti-gcc} version 3.8.0 with
\texttt{-O1}.
%
\todo{Check if this still holds. provide optimization flag}{Przemek} 
%
The source code for all benchmarks is released at \cite{coala_website}.
...
%
\todo{Provide concrete numbers for all applications}{Amjad}
%
