...
We also used an alternative setup where the MSP-TS430RGZ48C launchpad was for continuous power experiments. We used internal timer for measuring run time. For harvested energy, we used the ThingMagic Astra-EX RFID reader with power levels in the [17.25,30]\,dBm range. We used EDB~\cite{edb} for execution time measurement. Applications were cross-compiled using GCC and Clang, so that we can use LLVM for \sys compiler.\todo{Report on LLVM version}{Brandon}
