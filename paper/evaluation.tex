...
%List of figres by Brandon - I will map them to sections asap

%A plot showing performance for each application comparing bars for Chain, no coalescing/hand-tasked, coalescing/hand-tasked, no-coalescing/compiler, coalescing/compiler.  Performance depicted, normalized to one of the configurations (probably Chain).  This plot covers most of the eval dimensions that we care about.

%There are a few things we might change, if this plot has too much in one place:
...
%After the main plot, we need a sensitivity plot or two.  Two ideas come to mind:

%*Page size sweep, like you sent out for Cuckoo, with a separate "bathtub" curve for each app (normalized).

%*Task-size / cap-size sweep (remember we can vary either with approximately the same effect).  We only need, maybe, 3 task/cap size configurations here.  This plot is important because it shows VIPER's flexibility via coalescing (hopefully!)