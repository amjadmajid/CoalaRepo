...
\todo{rename legend entry 'user code' with 'useful task code'; rename y-axis to
'Time usage (\%)'; increase font by
two more points everywhere}{Amjad}}
...
\emph{task coalescing} and \emph{task downscaling}. We compared \sys's best
...
For each coalescing strategy from Section~\ref{sec:task_adaptation} (EB, EA,
...
energy-aware coalescing, ETA: energy- and task-aware coalescing. \todo{rename
legend entry 'task code' to 'user code'; rename y-axis to 'Load (\%)'}{Amjad}}
...
%	\centering
%	\includegraphics[width=0.5\columnwidth]{figures/coala_alpaca_gcc}
%    \caption{Application execution time with the ETA variant of \sys,
%normalized to the execution time with Alpaca, as a function of distance to the
...
application for Coala and Alpaca, normalized to the latter. The results show
...
the static tasks.} with neither Alpaca, nor Coala \emph{without the downscaling
...
shows how \sys's task downscaling adaptation mechanism eliminates this
non-termination, without changing the static tasks and without recompiling the
application.

In a separate experiment, we have enabled task downscaling in \sys and measured
