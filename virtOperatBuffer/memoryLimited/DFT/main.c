...
//////////////////// IMPORTANT ////////////////////////
/*
 *  This Application can not run CURRENTLY with IPOS virtual operational buffer
 *  because the buffers support only integer numbers
 */
//////////////////////////////////////////////////////////
...
void discTimeSign()
{
    ...
    uint16_t in_n = RVAR(n);
    uint16_t in_x_n = RVAR(x[in_n]);
...
void dft_real() {
    ...
    uint16_t in_n = RVAR(n);
    uint16_t in_Xre_k = RVAR(Xre[in_k]);
...
void dft_im() {
    ...
    uint16_t in_k = RVAR(k);
    uint16_t in_n = RVAR(n);
    uint16_t in_Xim_k = RVAR(Xim[in_k]);
