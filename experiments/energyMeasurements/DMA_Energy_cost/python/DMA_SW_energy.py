...
def plot(xAxis, voltMeans, symb, labels):
	...
	for val in voltMeans:
		plt.plot( xAxis ,val, symb[i], label=labels[i], markeredgewidth=1.5, markerfacecolor='0.75', markeredgecolor=colors[i])
		i+=1
	plt.legend(loc='best')
	plt.xlabel("Words")
...
def main():
	...
	plot(xAxis ,[dma_volt_mean_scaled, pt_volt_mean_scaled] , ['g-o','b:>'], ['DMA','SW'] )
