...
__p uint8_t* _v_return_to;	
__p char _v_result[LENGTH];
__p unsigned char _v_ukey[16];
__p unsigned _v_index;
__p uint32_t _v_index2;
__p uint32_t _v_s0[256];
__p uint32_t _v_s1[256];
__p uint32_t _v_s2[256];
__p uint32_t _v_s3[256];
__p uint32_t _v_key[18];
__p uint32_t _v_input[2];
__p unsigned _v_n;
__p enum task_index _v_next_task;
__p unsigned char _v_iv[8];
...
void task_encrypt() {
	...
	//	unsigned index = *READ(P(_v_index));
	//	uint8_t* return_to;
	...
	for (index = 0; index < 17; ++index) {
		...
		//PRINTF("addr of s0: %x\r\n", &s0);
		//print_long(s0);
		s1 = P(_v_s1[((r>>16L)&0xff)]);
		s2 = P(_v_s2[((r>> 8L)&0xff)]);
		s3 = P(_v_s3[((r     )&0xff)]);
		l^=(((	s0 + 
						s1)^ 
					s2)+ 
...
void task_start_encrypt() {
	...
	//	n = *READ(P(_v_n));
	if (P(_v_n) == 0) {
		uint32_t tmp_input_0;
		tmp_input_0 =((unsigned long)(P(_v_iv[0])))<<24L;
		...
		os_jump(OFFSET(t_start_encrypt, t_start_encrypt3));
		return;
	}
}
void task_start_encrypt2() {
	unsigned char tmp_iv[8];
	tmp_iv[0] = (unsigned char)(((P(_v_input[0]))>>24L)&0xff);
	tmp_iv[1] = (unsigned char)(((P(_v_input[0]))>>16L)&0xff);
	tmp_iv[2] = (unsigned char)(((P(_v_input[0]))>> 8L)&0xff);
	tmp_iv[3] = (unsigned char)(((P(_v_input[0]))     )&0xff);
	tmp_iv[4] = (unsigned char)(((P(_v_input[1]))>>24L)&0xff);
	tmp_iv[5] = (unsigned char)(((P(_v_input[1]))>>16L)&0xff);
...
#endif
	os_jump(OFFSET(t_start_encrypt2, t_start_encrypt3));
	return;
}
...
void task_start_encrypt3() {
	...
	if (P(_v_index2) == LENGTH) {
		...
		os_jump(OFFSET(t_start_encrypt3, t_start_encrypt));
		return;
	}
}
...
int main(void) {
	...
	taskId tasks[] = {{task_init, 0},
		...
		{task_set_key2, 0},
		{task_encrypt, 0},
		{task_start_encrypt, 0},
		{task_start_encrypt2, 0},
		{task_start_encrypt3, 0}};
